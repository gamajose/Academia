...
class _StudentToolsPageState extends State<StudentToolsPage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    tabController = TabController(length: 4, vsync: this, initialIndex: widget.initialTab.clamp(0, 3));
    refresh();
  }
  ...
  Future<void> refresh() async {
    ...
    try {
      final results = await Future.wait([
        getJson('/api/student/account/overview'),
        getJson('/api/student/payments?limit=100'),
        getJson('/api/student/checkins?limit=100'),
        getJson('/api/student/notifications'),
      ]);
      if (!mounted) return;
      setState(() {
        overview = results[0];
        payments = results[1]['data'] as List<dynamic>? ?? [];
        checkins = results[2]['data'] as List<dynamic>? ?? [];
        notifications = results[3]['data'] as List<dynamic>? ?? [];
        message = 'Atualizado.';
      });
    } catch (error) {
  ...
  Color financialColor(String status) {
    switch (status) {
      case 'current':
        return Colors.green;
      case 'due':
  ...
  String financialLabel(String status) {
    switch (status) {
      case 'current':
        return 'Em dia';
      case 'due':
        ...
        Card(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                const Text('Meu plano', style: TextStyle(fontSize: 21, fontWeight: FontWeight.bold)),
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Icon(Icons.account_balance_wallet, color: financialColor(status)),
                    const SizedBox(width: 8),
                    Text(
                      financialLabel(status),
                      style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: financialColor(status)),
                    ),
                  ],
                ),
                const SizedBox(height: 10),
                if (pending == null)
                  const Text('Nenhuma mensalidade pendente.')
                else ...[
        ...
        Card(
          child: Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Minha frequencia', style: TextStyle(fontSize: 21, fontWeight: FontWeight.bold)),
                const SizedBox(height: 10),
  ...
  Widget paymentsTab() {
    if (payments.isEmpty) return const Center(child: Text('Nenhum pagamento registrado.'));
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (_, index) {
          ...
          final overdue = Number.tryParse('${item['overdue_days']}') ?? 0;
          return Card(
            child: ListTile(
              leading: Icon(
                status == 'paid' ? Icons.check_circle : overdue > 0 ? Icons.warning_amber : Icons.schedule,
                color: status == 'paid' ? Colors.green : overdue > 0 ? Colors.red : Colors.orange,
              ),
              title: Text('${money(item['amount_cents'])} - ${status.toUpperCase()}'),
              subtitle: Text(
                'Vencimento: ${dateOnly(item['due_date'])}'
                '${item['paid_at'] == null ? '' : '\nPago em: ${dateTimeText(item['paid_at'])}'}'
                '${item['method'] == null ? '' : '\nForma: ${item['method']}'}'
  ...
  Widget checkinsTab() {
    if (checkins.isEmpty) return const Center(child: Text('Nenhuma entrada registrada.'));
    return RefreshIndicator(
      onRefresh: refresh,
      child: ListView.builder(
        padding: const EdgeInsets.all(12),
        ...
        itemBuilder: (_, index) {
          ...
          return Card(
            child: ListTile(
              ...
              subtitle: Text(
                '${item['device_name'] ?? 'Entrada manual'}'
                '\nOrigem: ${item['source'] ?? '-'}'
  ...
  Future<void> markAllRead() async {
    try {
      await postJson('/api/student/notifications/read', {'all': true});
      await refresh();
    } catch (error) {
    ...
    return Column(
      children: [
        Padding(
          ...
          child: Row(
            children: [
              Expanded(child: Text('${notifications.length} notificacao(oes)', style: const TextStyle(fontWeight: FontWeight.bold))),
              TextButton.icon(onPressed: markAllRead, icon: const Icon(Icons.done_all), label: const Text('Marcar lidas')),
            ],
          ),
        ),
        Expanded(
                    ...
                    itemBuilder: (_, index) {
                      ...
                      return Card(
                        child: ListTile(
                          ...
                          title: Text(
                            item['title']?.toString() ?? 'Notificacao',
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          TabBarView(
            controller: tabController,
            children: [planTab(), paymentsTab(), checkinsTab(), notificationsTab()],
          ),
          if (loading) const LinearProgressIndicator(),
          Positioned(left: 12, right: 12, bottom: 6, child: Text(message, textAlign: TextAlign.center)),
        ],
      ),
    );
  }

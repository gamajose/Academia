...
async function editMember(member) {
  ...
  await runAction(() => request('/api/members/update', {
    method: 'POST',
    body: JSON.stringify({ member_id: member.id, name, email, phone })
  }), 'Aluno atualizado.');
}

async function toggleMember(member) {
  const action = member.status === 'active' ? 'deactivate' : 'activate';
  await runAction(() => request(`/api/members/${action}`, {
    method: 'POST',
    body: JSON.stringify({ member_id: member.id })
  }), member.status === 'active' ? 'Aluno desativado.' : 'Aluno ativado.');
}
...
async function editPlan(plan) {
  ...
  await runAction(() => request('/api/plans/update', {
    method: 'POST',
    body: JSON.stringify({ plan_id: plan.id, name, price_cents: Number(price || plan.price_cents), duration_days: Number(days || plan.duration_days) })
  }), 'Plano atualizado.');
}

async function togglePlan(plan) {
  const action = plan.is_active ? 'deactivate' : 'activate';
  await runAction(() => request(`/api/plans/${action}`, {
    method: 'POST',
    body: JSON.stringify({ plan_id: plan.id })
  }), plan.is_active ? 'Plano desativado.' : 'Plano ativado.');
}
...
async function cancelMembership(membership) {
  ...
  await runAction(() => request('/api/memberships/cancel', {
    method: 'POST',
...
async function markPaymentPaid(paymentId) {
  await runAction(() => request('/api/payments/mark-paid', {
    method: 'POST',
...
async function cancelPayment(payment) {
  ...
  await runAction(() => request('/api/payments/cancel', {
    method: 'POST',
...
async function createCheckin() {
  await runAction(() => request('/api/checkins', {
    method: 'POST',
...
byId('refresh-button').addEventListener('click', () => runAction(refreshDashboard, 'Dashboard atualizado.'));
